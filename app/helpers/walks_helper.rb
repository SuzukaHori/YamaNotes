...
module WalksHelper
  def walk_status_label(walk)
    ...
    if walk.finished?
      content_tag :span, t('helpers.walks.status.finished'), class: "#{base_classes} bg-green-50 text-green-700 ring-green-600/20"
    elsif walk.active?
      content_tag :span, t('helpers.walks.status.active'), class: "#{base_classes} bg-blue-50 text-blue-700 ring-blue-700/10"
    else
      content_tag :span, t('helpers.walks.status.retired'), class: "#{base_classes} bg-gray-50 text-gray-600 ring-gray-500/10"
    end
  end
  ...
  def convert_to_local_time(m_seconds)
    ...
    t('helpers.walks.duration', hours:, minutes: minutes.to_i)
  end
end
